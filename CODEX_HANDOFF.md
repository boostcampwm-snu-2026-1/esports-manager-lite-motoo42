...
- 유저 팀 관련 스토브리그 로그는 `isUserTeamRelated`로 하이라이트하고 메시지함 중요/이적 메시지로 복사한다.
- 28일차: 최종 로스터 검증 후 다음 시즌 진입.
- `OffseasonOffer`는 future `transfer` kind까지 고려한 구조.
- AI 팀은 FA 풀 기반으로 포지션 부족을 자동 보강하며, 여러 AI 팀이 같은 FA에게 제안한 뒤 점수로 승자를 고르는 AI-AI 영입 경쟁 기록을 남긴다. AI-유저 트레이드 협상은 후속 고도화로 남김.
