...
LCK 팀의 감독이 되어 스토브리그부터 한 시즌을 직접 운영해볼 수 있다.

- **즐기는 법**: 시작 화면에서 LCK 10개 팀 중 하나를 고르고, 1군 5인 선발·계약·훈련·전략을 설정한 뒤 "진행"을 눌러 경기와 대회를 치른다. 라이브 매치 화면에서 밴픽과 문자중계로 경기를 관전할 수 있다.
- **시즌 데이터 저장**: 진행 상황은 MongoDB에 저장된다. 수동 저장/불러오기와 자동 저장을 지원하고, 브라우저별로 저장 공간이 자동 분리되어 같은 링크에서도 개인 세이브가 섞이지 않는다.
- **가이드**: 게임 안에서 화면별 단계 가이드를 제공해, 홈·로스터·스토브리그·대회 등에서 무엇을 하면 되는지 바로 확인할 수 있다.
